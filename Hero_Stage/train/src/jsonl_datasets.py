...
def make_train_dataset_T5only(args, tokenizer, accelerator=None):
    if args.train_data_dir is not None:
        print("load_data")
        dataset = load_dataset('json', data_files=args.train_data_dir)
    ...
    tokenizer_t5 = tokenizer[0]

    def tokenize_prompt_t5(examples):
        captions = []
        for caption in examples[caption_column]:
            if isinstance(caption, str):
                if random.random() < 0.1:
    ...
    def preprocess_train(examples):
        ...
        target_images = [load_image_safely(image_path, args.cond_size) for image_path in examples[target_column]]
        _examples["pixel_values"] = [train_transforms(image, noise_size) for image in target_images]
        _examples["token_ids_t5"] = tokenize_prompt_t5(examples)
        return _examples
...
def collate_fn(examples):
    ...
    token_ids_t5 = torch.stack([example["token_ids_t5"].clone().detach().clone().detach() for example in examples])
