...
class BriaPipeline(DiffusionPipeline, FluxLoraLoaderMixin, FromSingleFileMixin):
    r"""
    ...
    model_cpu_offload_seq = "text_encoder->text_encoder->transformer->vae"
    ...
    def __init__(
        ...
        self.vae.config.scaling_factor = 1.0 if self.vae.config.scaling_factor == None else self.vae.config.scaling_factor
        self.vae.config.shift_factor = 0.0 if self.vae.config.shift_factor == None else self.vae.config.shift_factor
    ...
    def encode_prompt(
        ...
        Args:
            ...
            prompt_2 (`str` or `List[str]`, *optional*):
                The prompt or prompts to be sent to the `` and `text_encoder`. If not defined, `prompt` is
