...
def load_stable_diffusion(sd_version='2.1', precision_t=torch.float32, device="cuda"):
    if sd_version == '2.1':
        ...
        model_key = "/root/workspace/_ckpt/huggingface/stabilityai/stable-diffusion-v1-5"
    elif sd_version == 'xl':
        pass
...
def attention_op(
    ...
    if attention_mask_ls is not None:
        ...
        # h_w_rate = cos_map_matrix.shape[1]/cos_map_matrix.shape[2]
        # hw = query.shape[1]
        # h = int(math.sqrt(h_w_rate*hw))
        # w = int(h/h_w_rate)
        # h_base = cos_map_matrix.shape[1]
        # if abs(h-h_base)<1:
        #     attention_mask = cos_map_matrix
        # elif abs(h-int(h_base/2))<2:
        ...
        # attention_mask = torch.eye(attention_mask.shape[1]).unsqueeze(0).to(hidden_states.dtype).to(hidden_states.device)
    ...
    if attention_probs is None:
        attention_probs, attention_scores = get_attention_scores(attn, query, key, attention_mask, beta=beta)
        attention_probs_womask, attention_scores_womask = get_attention_scores(attn, query, key, None, beta=beta)
    ...
    if mode == "to_I":
        ...
        result_dict["before_IV"].append(torch.abs(hidden_states).mean())
        head, n, _ = attention_probs.shape

        attention_probs = torch.eye(n, dtype=attention_probs.dtype, device=attention_probs.device).unsqueeze(0).expand(head, -1, -1)
        hidden_states = torch.bmm(attention_probs, value)
        
        result_dict["after_IV"].append(torch.abs(hidden_states).mean())
    else:
        hidden_states = torch.bmm(attention_probs, value)
...
def get_attention_scores(
        attn, query: torch.Tensor, key: torch.Tensor, attention_mask: torch.Tensor = None, beta=1.0,
    ) -> torch.Tensor:
        r"""
        ...
        if attention_mask is None:
            ...
            # beta=1, alpha=0.07, 0.11, 0.15... as default
            beta = beta
            if beta < 0:
                baddbmm_input = torch.ones_like(baddbmm_input) - baddbmm_input
